...
public class LockSetupActivity extends Activity implements
	...
	private static final int STEP_1 = 1; // 寮�濮�
	private static final int STEP_2 = 2; // 绗竴娆¤缃墜鍔垮畬鎴�
	private static final int STEP_3 = 3; // 鎸変笅缁х画鎸夐挳
	private static final int STEP_4 = 4; // 绗簩娆¤缃墜鍔垮畬鎴�
	// private static final int SETP_5 = 4; // 鎸夌‘璁ゆ寜閽�
	...
	public void onClick(View v) {
		...
		case R.id.left_btn:
			if (step == STEP_1 || step == STEP_3 || step == STEP_4) {
				finish();//关闭activity
			} else if (step == STEP_2) {//状态2，说明是输入了
				step = STEP_1;
				updateView();
			}
			break;
		...
		case R.id.right_btn:
			if (step == STEP_2) {
				...
				Intent intent = new Intent(this, LockActivity.class);//次数进入受保护的activity
